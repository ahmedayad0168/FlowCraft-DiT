...
def tiny_model(**overrides) -> FlowCraftMMDiT:
    return FlowCraftMMDiT(tiny_config(**overrides))


def batch(model: FlowCraftMMDiT, b: int = 2, hw: int = 8, n_txt: int = 5):
    x = torch.randn(b, model.config.in_channels, hw, hw)
    t = torch.rand(b)
    text = torch.randn(b, n_txt, model.config.txt_dim)
    return x, t, text
...
def test_forward_rejects_wrong_channel_count():
    ...
    x, t, text = batch(model)
    with pytest.raises(ValueError):
        model(torch.randn(2, 3, 8, 8), t, text)
...
def test_forward_rejects_indivisible_latent():
    ...
    with pytest.raises(ValueError):
        model(torch.randn(2, 4, 7, 7), t, text)
...
def test_pooled_text_required_when_configured():
    ...
    with pytest.raises(ValueError):
        model(x, t, text)
    assert model(x, t, text, pooled_text=torch.randn(2, TXT_DIM)).shape == x.shape
...
def test_timestep_embedder_shape_and_odd_dim_padding():
    ...
    odd = TimestepEmbedder.sinusoidal_embedding(torch.rand(4), dim=7)
    assert odd.shape == (4, 7)


def test_adaln_zero_is_identity_at_init():
    adaln = AdaLNZero(16)
    shift, scale, gate, *_ = adaln(torch.randn(2, 16))
...
def test_rope_preserves_norm_and_is_cached():
    ...
    cos, sin = rope(4, 4, device=torch.device("cpu"), dtype=torch.float32)
    ...
    assert rope(4, 4, device=torch.device("cpu"), dtype=torch.float32)[0] is cos
...
def test_cfm_target_velocity_is_path_derivative():
    ...
    x0, x1 = torch.randn(2, 4, 8, 8), torch.randn(2, 4, 8, 8)
    t = torch.full((2,), 0.25)
    dt = 1e-3
...
def test_cfm_loss_weighting_and_prediction_return():
    ...
    loss, pred, extra = ConditionalFlowMatcher().compute_loss(
        model, x, text, t=t, loss_weight=torch.zeros(2), return_prediction=True
    )
...
def test_euler_sample_shape_and_determinism():
    ...
    def run():
        return sampler.sample(
            ...
            generator=torch.Generator().manual_seed(0), device=torch.device("cpu"),
        )
...
def test_euler_honours_x_init():
    ...
    out = EulerSampler(num_steps=1).sample(model=model, text_embeds=text, x_init=x0)
...
def test_euler_restores_training_mode_and_requires_inputs():
    ...
    _, _, text = batch(model)
    EulerSampler(num_steps=2).sample(model=model, x_shape=(2, 4, 8, 8), text_embeds=text)
...
def test_euler_cfg_batches_masks_alongside_embeddings():
    ...
    out = EulerSampler(num_steps=2).sample(
        model=model, x_shape=(2, 4, 8, 8), text_embeds=text, null_text_embeds=torch.zeros_like(text),
        cfg_scale=3.0, model_kwargs={"txt_mask": mask}, null_model_kwargs={"txt_mask": mask},
    )
...
def test_euler_differentiable_keeps_graph():
    ...
    _, _, text = batch(model)
    out = EulerSampler(num_steps=2).sample(
        model=model, x_shape=(2, 4, 8, 8), text_embeds=text, differentiable=True
    )
...
def test_euler_returns_trajectory():
    ...
    out, trajectory = EulerSampler(num_steps=3).sample(
        model=model, x_shape=(2, 4, 8, 8), text_embeds=text, return_trajectory=True
    )
...
def test_reflow_pair_shares_the_same_noise():
    ...
    x0, x1_pred = pipeline.generate_reflow_pair(
        x_shape=(2, 4, 8, 8), text_embeds=text, device=torch.device("cpu")
    )
...
def test_reflow_student_loss_is_finite():
    ...
    x0, x1_pred = pipeline.generate_reflow_pair(x_shape=(2, 4, 8, 8), text_embeds=text)
...
def test_logit_normal_sampler_is_reproducible():
    sampler = LogitNormalSampler()
    a = sampler.sample(8, generator=torch.Generator().manual_seed(1))
    b = sampler.sample(8, generator=torch.Generator().manual_seed(1))
